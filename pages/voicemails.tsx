...
export default function VoicemailInboxPage() {
  ...
  return <>
    ...
    <main className="page"><div className="shell">
      <div className="heading"><div><p className="eyebrow">AI4 CONTACT CENTER · DEVELOPMENT AGENT OPERATIONS</p><h1>Voicemail Inbox</h1><p>Automatic monitoring is disabled. Query voicemail only when you need a development snapshot{lastChecked ? ` · Last checked ${lastChecked}` : ''}.</p></div><div className="headingActions"><button onClick={() => refresh().catch((e: Error) => setError(e.message))} disabled={loading}>{loading ? 'Checking…' : 'Check Voicemail'}</button><button onClick={() => router.push('/agent-workspace')}>Agent Workspace →</button></div></div>
      {error && <div className="error">{error}</div>}
      {loading ? <p className="muted">Checking voicemail…</p> : !checked ? <div className="empty standalone">No voicemail query has been run. Use Check Voicemail when needed.</div> : <div className="layout">
        <aside>{rows.length === 0 ? <div className="empty">No voicemail messages in the current snapshot.</div> : rows.map((row) => <button key={row.id} className={selected?.id === row.id ? 'active' : ''} onClick={() => setSelectedId(row.id)}><strong>{row.caller_identifier ?? 'Unknown caller'}</strong><span>{row.metadata?.temporalState ?? 'voicemail'} · {row.callback_status}</span><small>{new Date(row.received_at).toLocaleString()}</small></button>)}</aside>
        <section>{!selected ? <div className="empty">No voicemail selected.</div> : <div className="card">
