...
const META: Record<ChannelKey, { title: string; subtitle: string; runtime: string; primary: string; secondary?: string }> = {
  voice: { title: 'Voice Operations', subtitle: 'Incoming calling, IVR/auto-attendant readiness, queue routing and live interaction visibility.', runtime: 'Twilio Voice', primary: '/builder', secondary: '/voice-attendant' },
  sms: { title: 'SMS Operations', subtitle: 'Inbound Twilio messaging, routing, automated reply handling and canonical interaction visibility.', runtime: 'Twilio Messaging', primary: '/agent-workspace?channel=sms' },
  chat: { title: 'Web Chat Operations', subtitle: 'Site chat runtime, queue/agent routing and canonical conversation visibility.', runtime: 'AI4 Web Chat', primary: '/web-chat' },
};
...
export default function ChannelOperationsPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => load().catch((e: Error) => active && setError(e.message)), 15000);
    return () => { active = false; window.clearInterval(timer); };
  }, [router, router.isReady, key, meta]);

  const facts = useMemo(() => [
    ['Queue', health?.details.queueReady ? 'Ready' : 'Not ready'],
    ['Flow Version', health?.details.flowVersionReady ? 'Ready' : 'Not ready'],
    ['Runtime Routing', health?.details.runtimeRouting ? 'Ready' : 'Not ready'],
    ['Available Agent', health?.details.agentReady ? 'Available' : 'None available'],
  ], [health]);
  ...
  return <>
    ...
    <main className="page"><div className="shell">
      ...
      <section className="hero">
        <div><p className="eyebrow">AI4 CONTACT CENTER · {meta.runtime.toUpperCase()}</p><h1>{meta.title}</h1><p>{meta.subtitle}</p></div>
        <div className="health"><small>CHANNEL STATUS</small><strong style={{color:tone(health?.status ?? 'unknown')}}>{(health?.status ?? 'unknown').replace(/_/g,' ').toUpperCase()}</strong><span>{health?.details.service ?? meta.runtime}</span></div>
      </section>
      ...
      <div className="sectionTitle"><div><small>RECENT {meta.title.toUpperCase()}</small><h2>Canonical interactions</h2></div><span>Auto-refresh · 15 sec</span></div>
      <section className="list">
        {interactions.length === 0 ? <div className="empty">No recent {key} interactions are currently available.</div> : interactions.map((item) => <Link key={item.id} href={`/agent-workspace?channel=${key}&interaction=${item.id}`} className="row">
