...
| 2 | **Intake webhook was hard-wired to Apropos's tenant with no auth — FIXED in code 2026-09-24 (PR #29); enforcement pending.** The tenant and actor now come from a per-tenant key in the `x-ai4cc-intake-key` header (SHA-256 stored in `ai4cc_integrations`, `provider='elevenlabs'`, `integration_type='intake_webhook'`, `config={key_sha256, actor_user_id}`). Wrong key → 401. Unkeyed calls are still accepted **for the Apropos tenant only** until the live demo agent's two ElevenLabs tools send the header and `AI4CC_INTAKE_ALLOW_UNKEYED=false` is set in Netlify. | Code + preview tests | New customers get their own key row; leads can no longer land in another tenant's CRM. |
...
3. **Per-tenant webhook key + tenant-bound intake webhook.** *(Webhook side done, PR #29; the key row + ElevenLabs header per customer are provisioning-script steps.)*
